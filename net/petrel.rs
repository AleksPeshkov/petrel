...
fn main() {
    ...
    let mut trainer = ValueTrainerBuilder::default().use_threads(CPU_THREADS/2)
        ...
        .save_format(&[
            ...
            SavedFormat::id("l1b").quantise::<i32>(QF*QB*WDL),
        ])
    ...
    let batch_size = 16_384 /2;
    let batches_per_superbatch = 6_104 *2;
    let initial_lr = 1e-3;
    let final_lr = 1e-6;
    ...
    trainer.optimiser.set_params_for_weight("l1w", AdamWParams{ decay: 0.01, min_weight: -f_wdl, max_weight: f_wdl, ..Default::default() });

    let schedule = TrainingSchedule {
        net_id: "1b1".to_string(),
        ...
        wdl_scheduler: wdl::CosineDecayWDL { start: 0.0, end: 0.15, final_superbatch },
