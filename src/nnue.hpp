...
class CACHE_ALIGN Accumulator {
    ...
    class AccumulatorSide {
        ...
        constexpr static Nnue::FeautureIndex fi(Side si, PieceType ty, Square sq) {
            constexpr PieceType::arrayOf<PieceType::_t> pieceType = {Pawn, Knight, Bishop, Rook, Queen, King};
            return Nnue::FeautureIndex{ 6*64*static_cast<Side::_t>(si) + 64*pieceType[ty] + static_cast<Square::_t>(~sq) };
        }

        constexpr void move(Index i, Side si, Square from, PromoType toType, Square to) {
            v[i] -= w[fi(si, PieceType{Pawn}, from)][i];
            v[i] += w[fi(si, PieceType{static_cast<PromoType::_t>(toType)}, to)][i];
        }

        constexpr void move(Index i, Side si, PieceType ty, Square from, Square to) {
            v[i] -= w[fi(si, ty, from)][i];
            v[i] += w[fi(si, ty, to)][i];
        }

        constexpr void capture(Index i, Side si, NonKingType captured, Square to) {
            v[i] -= w[fi(~si, PieceType{static_cast<NonKingType::_t>(captured)}, to)][i];
        }
    ...
    public:
        ...
        void ep(Side si, Square from, Square to, Square ep) {
            for (auto i : range<Index>()) {
                move(i, si, PieceType{Pawn}, from, to);
        ...
        void castle(Side si, Square kingFrom, Square kingTo, Square rookFrom, Square rookTo) {
            for (auto i : range<Index>()) {
                move(i, si, PieceType{King}, kingFrom, kingTo);
                move(i, si, PieceType{Rook}, rookFrom, rookTo);
            }
        }
    };
