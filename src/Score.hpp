...
struct Score {
    ...
    static constexpr Score fromTt(unsigned n, Ply ply) {
        ...
        if (score < MinEval - ply) {
            score.assertMate(); score = score + ply; score.assertMate();
        }
        else if (MaxEval + ply < score) {
            score.assertMate(); score = score - ply; score.assertMate();
        }
        else {
            score.assertEval();
...
public:
    ...
    constexpr int count(NonKingType ty) const {
        return v.s.count[ty];
    }

    // any queen, rook or pawn
    constexpr bool hasMatingPieces() const {
        return count(NonKingType{Queen}) + count(NonKingType{Rook}) + count(NonKingType{Pawn}) > 0;
    }
