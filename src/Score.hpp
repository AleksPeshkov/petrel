...
public:
    ...
    explicit constexpr Score (_t e) : v_{e} {}
    ...
    constexpr friend Score operator + (Arg a, Arg b) { a.assertEval(); b.assertEval(); return Score::clampEval(a.v_ + b.v_); }
    constexpr friend Score operator - (Arg a, Arg b) { a.assertEval(); b.assertEval(); return Score::clampEval(a.v_ - b.v_); }
    constexpr friend Score operator + (Arg a, Ply p) { a.assertMate(); Score r{static_cast<_t>(a.v_ + p.v())}; r.assertMate(); return r; }
    constexpr friend Score operator - (Arg a, Ply p) { a.assertMate(); Score r{static_cast<_t>(a.v_ - p.v())}; r.assertMate(); return r; }
