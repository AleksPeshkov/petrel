...
public:
    ...
    // mask less valuable pieces
    PiMask goodKillers(PieceType ty) const { assert (!ty.is(King)); return anyOf(GoodKillersMask[ty]); }

    // mask of equal of less valuable pieces
    PiMask notBadKillers(PieceType ty) const { assert (!ty.is(King)); return anyOf(NotBadKillersMask[ty]); }
