...
ReturnStatus Node::negamax(Node* child) const {
    ...
    if (beta <= childScore) {
        ...
        if (child->draft < draft-1 && currentMove) {
            child->draft = draft-1; // revert reduction
            return negamax(child);
        }
        // fix possibly wrong mate score because null move is invalid move
        //TRICK: this is WRONG when mate-distance pruning is NOT used
    ...
    if (alpha < childScore) {
        ...
        if (!child->isPv) {
            ...
            assert (currentMove);
            child->draft = draft-1; // revert possible reductions
            return negamax(child);
        }
...
ReturnStatus Node::search() {
    ...
    if (parent && !inCheck()) {
        ...
        // countermove heuristic: refutation of the last opponent's move
        if (opMove) {
            for (int i = 1; i < decltype(root.counterMove)::Size; ++i) {
                auto move = root.counterMove.get(i,
                    parent->colorToMove(), parent->MY.typeAt(opMove.from()), opMove.to()
                );
                if (isLegalMove(move)) {
        ...
        if (grandParent) {
            Move myMove = grandParent->currentMove;
            if (myMove) {
                for (int i = 1; i < decltype(root.followMove)::Size; ++i) {
                    auto move = root.followMove.get(i,
                        grandParent->colorToMove(), grandParent->MY.typeAt(myMove.from()), myMove.to()
                    );
                    if (isLegalMove(move)) {
    ...
    // Late Move Pruning: !inCheck() && !isPv && draft <= 2
    bool lmp = !inCheck() && !isPv && draft <= 2;
