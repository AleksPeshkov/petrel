...
template <IndexLike Index> static constexpr auto range() {
...
public:
    ...
    constexpr void assertOk() const { assert (v_ == None || +from() != 0 || +to() != 0); } // check for canonical null move
    constexpr bool none() const { return v_ == None; }
    constexpr bool any() const { return !none(); }
...
public:
    ...
    constexpr bool isSpecial() const { return historyType().is(HistoryPawn); }
