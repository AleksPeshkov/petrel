...
public:
    union _t {
        struct {
            ...
            unsigned pieceMat:7; // sum of non pawn pieces material points (pawn = 1)
...
protected:
    inline static constexpr signed openingMat[] = { 1025, 477, 365, 337, 82, 100 }; // PeSTO piece opening values
    inline static constexpr signed endgameMat[] = { 936, 512, 297, 281, 94, 100 }; // PeSTO piece endgame values
    inline static constexpr unsigned pieceMat[] = { 10, 5, 3, 3, 0, 0 }; // for game phase, total sum is 32
    inline static constexpr unsigned totalMat[] = { 12, 6, 4, 4, 1, 0 }; // for exchange evaluations
    inline static constexpr unsigned pawns[]    = {  0, 0, 0, 0, 1, 0 };
    inline static constexpr unsigned knights[]  = {  0, 0, 0, 1, 0, 0 };
    inline static constexpr unsigned bishops[]  = {  0, 0, 1, 0, 0, 0 };
    inline static constexpr unsigned rooks[]    = {  0, 1, 0, 0, 0, 0 };
    inline static constexpr unsigned queens[]   = {  1, 0, 0, 0, 0, 0 };

    inline static constexpr signed openingPst[PieceType::Size][Square::Size] = {
        { // Queen
    ...
    inline static constexpr signed endgamePst[PieceType::Size][Square::Size] = {
        { // Queen
...
public:
    constexpr PieceSquareTable () {
        FOR_EACH (PieceType, ty) {
            FOR_EACH (Square, sq) {
                pst[ty][sq] = {{
                    static_cast<unsigned>(openingMat[ty] + openingPst[ty][sq]),
                    static_cast<unsigned>(endgameMat[ty] + endgamePst[ty][sq]),
                    pieceMat[ty],
                    totalMat[ty],
                    pawns[ty], knights[ty], bishops[ty], rooks[ty], queens[ty],
                }};
            }
        }
    }
...
public:
    ...
    static Score evaluate(const Evaluation& my, const Evaluation& op) {
        constexpr const unsigned PieceMatMax = 32; // initial sum of non pawn pieces material points
