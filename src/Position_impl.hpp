...
void Position::nnUpdate() {
    ...
    hm_[My] = MY.sqKing().hMask();
    for (auto pi : MY.any()) {
        fi[PieceList{count++}] = {side_to_move_t::My, MY.piece(pi), MY.sq(pi)^hm_[My]};
    }

    //TRICK: flip pieces squares perspective for opposite side
    auto m = ~hm_[My];
    for (auto pi : OP.any()) {
        fi[PieceList{count++}] = {side_to_move_t::Op, OP.piece(pi), OP.sq(pi)^m};
    }
...
bool Position::makeMove(const Position& parent, Square from, Square to, auto&& flipPrefetch) {
    ...
    if (MY.isKing(from)) [[unlikely]] {
        ...
        if (OP.has(~to)) {
            ...
            if constexpr (Flags & WithEval) { nnUpdate(parent, Square::crossed_middle(from, to), {My,King,from}, {My,King,to}, {Op,captured,to}); }
            ...
            if constexpr (Flags & WithEval) { nnUpdate(parent, Square::crossed_middle(from, to), {My,King,from}, {My,King,to}); }
...
// non-pawn non-king move (but can be castling):
    ...
    if (MY.isCastling(pi)) [[unlikely]] {
        if (MY.isKing(to)) [[likely]] {
            ...
            if constexpr (Flags & WithEval) { nnUpdate(parent, Square::crossed_middle(kingFrom, kingTo), {My,King,kingFrom}, {My,King,kingTo}, {My,Rook,rookFrom}, {My,Rook,rookTo}); }
