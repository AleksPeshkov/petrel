...
istream& UciSearchLimits::uciGo(istream& in, Side white, bool extraTime) {
...
void UciSearchLimits::uciStop() {
    stop_.store(true, std::memory_order_release);
    infinite_.store(false, std::memory_order_relaxed);
    ponder_.store(false, std::memory_order_relaxed);
}

void UciSearchLimits::uciPonderhit() {
    ponder_.store(false, std::memory_order_relaxed);
}
...
void Uci::loadEvalFile(const std::string& fileName) {
    ...
    if (fileSize == std::streamsize(-1)) {
        log("#Error reading EvalFile " + fileName + ": file size is unknown");
        return;
    }

    if (static_cast<size_t>(fileSize) != sizeof(nnue)) {
        log("#EvalFile size mismatch, expected " + std::to_string(sizeof(nnue)) + ", file size " + std::to_string(fileSize));
        return;
    }
...
void Uci::go() {
    ...
    newSearch();
    limits.uciGo(inputLine, position_.sideOf(White), isNewGame);
...
void Uci::bench(std::string& goLimits) {
    ...
    static std::string_view positions[][2] = {
        ...
        {"rk6/p1rB2p1/P5Kp/4B3/1p6/8/8/8 w - - 0 2", "bm g6f7; id mate#4 talkchess.com/viewtopic.php?t=71596"}, // depth 22
    ...
    for (auto pos : positions) {
        ...
        limits.uciGo(is, position_.sideOf(White));
