...
// search all moves:
    ...
    if (isRoot()) {
        for (auto move : root.rootBestMoves) {
            if (move.none()) { break; }
            RETURN_CUTOFF (child->searchIfPossible(move));
        }
    }
    ...
    do {
        ...
        RETURN_CUTOFF (goodPawnsMovesTo(~(OP.bbSide() - OP.bbPawns()), 3_ply));
...
ReturnStatus Node::searchMove(HistoryMove move, Ply R) {
    ...
    if (rule50() < 2_ply || ply <= 2_ply) { assert (!isRoot()); zHash = {}; }
...
void refreshTtPv(const PositionMoves& p, const PrincipalVariation& pv, const Tt& tt) {
    ...
    for (HistoryMove move; (move = *pmoves++).any();) {
        ...
        *o = TtSlot{pos.z(), score, ply, ExactScore, depth, TtMove{move}};
        ++tt.writes;
